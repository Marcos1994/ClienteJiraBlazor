	...
	public class SprintProfile : Profile
	{
		public SprintProfile()
		{
			CreateMap<ClienteJiraAPI.Modelos.Entrada.Sprint, Negocio.Modelos.Entidades.Sprint>()
				...
				.ForMember(d => d.Inicio, opt =>
				{
					opt.PreCondition(o => !string.IsNullOrEmpty(o.startDate));
					opt.MapFrom(o => Convert.ToDateTime(o.startDate));
				})
				.ForMember(d => d.Fim, opt =>
				{
					opt.PreCondition(o => !string.IsNullOrEmpty(o.endDate));
					opt.MapFrom(o => Convert.ToDateTime(o.endDate));
				})
